...
calculate_winP_pre_post <- function(data, group_var, post_var, pre_var = NULL) {
  ...
  data <- data %>%
    arrange(!!sym(group_var), desc(!!sym(post_var)), if (!is.null(pre_var)) desc(!!sym(pre_var))) %>%
  ...
  if (!is.null(pre_var)) {
    model2 <- lm(winf ~ winf_pre + get(group_var), data = data)
  } else {
    model2 <- lm(winf ~ get(group_var), data = data)
  }
